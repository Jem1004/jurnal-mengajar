...
import { updateMataPelajaran, getMataPelajaran } from '@/app/actions/master'
import Link from 'next/link'
import { ArrowLeft, Save, BookOpen, Code } from 'lucide-react'

export default function EditMataPelajaranPage({ params }: { params: { id: string } }) {
  ...
  const [mataPelajaranData, setMataPelajaranData] = useState<any>(null)
  ...
  const loadMataPelajaran = async () => {
    ...
    if (result.success && result.data) {
      const mataPelajaran = result.data.find((mp: any) => mp.id === params.id)
      if (mataPelajaran) {
        setMataPelajaranData(mataPelajaran)
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateMataPelajaran(params.id, formDataObj)
    if (result.success) {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Edit Data Mata Pelajaran</h1>
                <p className="text-sm text-gray-500">Perbarui informasi mata pelajaran yang ada</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-custom-lg border border-gray-100 p-8">
          ...
          <div className="bg-blue-50 rounded-xl p-4 mb-6 border border-blue-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-blue-900">Mengedit: {mataPelajaranData.nama}</p>
                {mataPelajaranData.kode && (
                  <p className="text-sm text-blue-700">Kode: {mataPelajaranData.kode}</p>
                )}
              </div>
            </div>
          </div>
