...
import { createJadwal, getGuru, getKelas, getMataPelajaran } from '@/app/actions/master'
...
export default function TambahJadwalPage() {
  ...
  const loadData = async () => {
    const [guruResult, kelasResult, mataPelajaranResult] = await Promise.all([
      getGuru(),
      getKelas(),
      getMataPelajaran()
    ])
    ...
    if (guruResult.success && guruResult.data) {
      ...
      if (guruResult.data.length > 0) {
        setFormData(prev => ({ ...prev, guruId: guruResult.data[0].id }))
      }
    }

    if (kelasResult.success && kelasResult.data) {
      setKelasList(kelasResult.data)
      if (kelasResult.data.length > 0) {
        setFormData(prev => ({ ...prev, kelasId: kelasResult.data[0].id }))
      }
    }

    if (mataPelajaranResult.success && mataPelajaranResult.data) {
      setMataPelajaranList(mataPelajaranResult.data)
      if (mataPelajaranResult.data.length > 0) {
        setFormData(prev => ({ ...prev, mataPelajaranId: mataPelajaranResult.data[0].id }))
      }
    }
  }
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Atur jadwal pelajaran untuk sistem pembelajaran</p>
              </div>
            </div>
          </div>
        </div>
