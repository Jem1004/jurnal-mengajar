...
import { updateSiswa, getSiswa, getKelas } from '@/app/actions/master'
...
export default function EditSiswaPage({ params }: { params: { id: string } }) {
  ...
  const loadData = async () => {
    ...
    if (siswaResult.success && siswaResult.data) {
      const siswa = siswaResult.data.find((s: Siswa) => s.id === params.id)
      if (siswa) {
        setSiswaData(siswa)
        setFormData({
          nisn: siswa.nisn,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await updateSiswa(params.id, formDataObj)
    if (result.success) {
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Perbarui informasi data siswa yang ada</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-custom-lg border border-gray-100 p-8">
          ...
          <div className="bg-blue-50 rounded-xl p-4 mb-6 border border-blue-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-blue-900">Mengedit: {siswaData.nama}</p>
                <p className="text-sm text-blue-700">NISN: {siswaData.nisn}</p>
              </div>
            </div>
          </div>
